...
*/
    ...
    */
	...
	public void setTurnLocation(double loc) {
		...
		if (getTurnRelPos() >= 0) {
			if ( (base + (loc * FULL_ROTATION) ) - getTurnRelPos() < -FULL_ROTATION / 2) {
				base += FULL_ROTATION;
			} else if ( (base + (loc * FULL_ROTATION) ) - getTurnRelPos() > FULL_ROTATION / 2) {
				base -= FULL_ROTATION;
			}
			turn.set( (loc * FULL_ROTATION) + (base) );
		} else { 
			if ( (base - ( (1-loc) * FULL_ROTATION) ) - getTurnRelPos() < -FULL_ROTATION / 2) {
				base += FULL_ROTATION;
			} else if ( (base - ( (1-loc) * FULL_ROTATION) ) - getTurnRelPos() > FULL_ROTATION / 2) {
				base -= FULL_ROTATION;
			}
